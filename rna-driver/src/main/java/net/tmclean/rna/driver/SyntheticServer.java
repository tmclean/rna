...
{	
	...
	{	
		Class<?> clazz1 = RNA.newEndpoint().at( "/restTest" )
				   .withGet( "/test/{path}" )
				      .produces( "testPOJO0", MediaType.APPLICATION_JSON )
				      .takesQueryParam( "test", int.class )
				      .takesPathParam( "path", String.class )
				      .withSource( "{return new " + RNA.getModelClassName( "testPOJO0" ) + "($1, $2);}" )
				      .apply()
					.apply();
		
		Class<?> clazz2 = RNA.newEndpoint().at( "/restTest2" )
				   .withGet( "/test2/{path}" )
				      .produces( "testPOJO1", MediaType.APPLICATION_JSON )
				      .takesQueryParam( "test", int.class )
				      .takesPathParam( "path", String.class )
				      .withSource( "{return new " + RNA.getModelClassName( "testPOJO1" ) + "($1, $2);}" )
				      .apply()
					.apply();
