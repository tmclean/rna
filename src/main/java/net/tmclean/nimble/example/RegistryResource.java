...
public class RegistryResource 
{
	...
	{	
		...
		for( String modelName : Nimble.getModelNames() )
		{
			Map<String, Object> modelProps = new HashMap<>();
			
			modelProps.put( "className", Nimble.getModelClassName( modelName ) );
			
			Map<String, Object> modelMethods = new HashMap<>();
			
			for( Method m : Nimble.getModel( modelName ).getDeclaredMethods() )
			{
			...
			for( Field field : Nimble.getModel( modelName ).getDeclaredFields() )
			{
				fields.put( field.getName(), field.getType().getName() );
			}
			
			List<List<String>> constructors = new ArrayList<>();
			for( Constructor<?> cons : Nimble.getModel( modelName ).getConstructors() )
			{
